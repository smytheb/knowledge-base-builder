...
## Quick Start (no prior terminal experience required)

### 1. Install prerequisites

- **Claude Code** — Anthropic's CLI. Install instructions: <https://docs.claude.com/en/docs/claude-code/quickstart>
- **git** — for cloning this repo. macOS already has it. Windows: <https://git-scm.com/download/win>
...
Copy the repo's GitHub URL (green "Code" button → HTTPS), then in your terminal:
...
- **`/start`** — bootstrap a fresh repo. Interviews you for topic, scope, audience, depth, and success criteria, then writes the answers into `CLAUDE.md`. Run this once after cloning.
...
## Manual Setup (alternative to `/start`)

If you'd rather configure the topic by hand:
...
Inspired by [Andrej Karpathy's LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) idea — a personal wiki where an LLM does the writing and a human steers. This project takes that core and adds source-gathering, planning, verification, and orchestration so it scales to building a serious knowledge base on any topic.
